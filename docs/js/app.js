(() => {
    ...
    function buildBlogItem(item) {
        ...
        item.image1x ? blogItemTemplate += `<a href="${item.url}" class="item-blog__image item-blog__image-ibg">\n\t\t\t<img src="${item.image1x}" srcset="${item.image2x} 2x" alt="Image">\n\t\t</a>` : null;
    ...
    if (inputSubscript) inputSubscript.addEventListener("blur", (function(e) {
        if (inputSubscript.value.length > 0) btnSubmit.disabled = false; else btnSubmit.disabled = true;
    }));
    window["FLS"] = true;
    isWebp();
    addLoadedClass();
